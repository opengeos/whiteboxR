...
The results of runs of WhiteboxTools via the whitebox package wrapper methods are specified as file paths for output files (often rasters in GeoTIFF format, sometimes other formats; other cases shapefiles, HTML output, LiDAR-related files etc.)
...
You have the ability to compile WhiteboxTools yourself and install it anywhere for the `whitebox`package. See https://github.com/jblindsay/whitebox-tools for details on building from source.

Specify the path to `whitebox_tools` executable with `whitebox.exe_path` option, `wbt_init()` provides the standard means to set `whitebox.exe_path` for a session.

### `wbt_init()`

`wbt_init()` is used to set and check the executable path of the binary executable that commands are passed to. 
The executable path and other options are stored as package options, and can be overridden by system environment variables. A default value `wbt_exe_path(shell_quote = FALSE)` is passed when the `exe_path` argument is left unspecified.
...
Any package options other than `exe_path` as detailed in `?whitebox::whitebox` can be set with `wbt_init(...)` or `wbt_options()`.
...
If you would rather not set the path to `whitebox_tools` binary from `$PATH`, you can set `R_WHITEBOX_EXE_PATH` manually.

Replace the `Sys.which()` call with a custom path string as needed.
