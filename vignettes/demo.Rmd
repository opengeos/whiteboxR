...
In this vignette we will use the `terra` package for visualization. Just as easily we could have used `raster`, `stars` or other options available in the R ecosystem.
...
On Linux/Mac you can set `R_WHITEBOX_EXE_PATH` directly with `export R_WHITEBOX_EXE_PATH="/path/to/whitebox_tools"`.
 ...
 - Note that if you set your `$PATH` in you regular shell profile (`.profile`/`.bashrc`/`.zshrc`) then it will _not_ be sourced by RStudio by default.
...
For instance, "BreachDepressions" is used below to process a Digital Elevation Model (DEM) before we identify flow pathways. This algorithm is prefered over depression filling in most cases and uses Lindsay's (2016) algorithm.
