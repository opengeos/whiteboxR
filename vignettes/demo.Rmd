...
A way the package will automatically find your `whitebox_tools` executable is if it is in a directory that exists in your PATH. This requires that you have the Unix tool `which` or one of its analogues.
...
The package will detect when you have added the WhiteboxTools directory to your `PATH` environment variable.
...
We use the {terra} package to read the GeoTIFF outputs from WhiteboxTools.
