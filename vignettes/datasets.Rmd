...
The `wbttoolparameters` data set is a _data.frame_ with `r nrow(wbttoolparameters)` parameters and 13 variables:
 ...
 - `"argument_name"` - labels for `"pkgflags"`; **used as R function argument names**
...
```{r}
subset(wbttoolparameters, grepl("OptionList", parameter_type) & grepl("variant", argument_name))
```
