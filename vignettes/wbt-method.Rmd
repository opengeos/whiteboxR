...
```{r}
if (requireNamespace("raster") && requireNamespace("rgdal")) {
...
read into memory on creation. Reading vector data into R memory is
not needed to run tools, so `wbt_source()` provides a means to annotate objects 
...
The SpatVectorProxy provides a lightweight means to create an R object reference to a large file;
allowing the heavy lifting for that file to be done primarily by WhiteboxTools.
...
Likewise for a `SpatRaster` object input without a CRS.

```{r}
wbt("slope", dem = terra::rast(dem), output = file.path(tempdir(), "foo.tif"))
```
...
```{r}
araster <- terra::set.crs(terra::rast(dem), "EPSG:26918")
...
```{r}
wbt("add", 
    input1 = terra::set.crs(terra::rast(dem), "EPSG:26918"), 
    input2 = terra::rast(dem), 
